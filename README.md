...
- Idle / stale sessions are filtered out automatically, so the deck stays a **live view**. Presence keys on **recent activity** (the session transcript, appended every turn/tool/hook — the truthful signal), NOT on the recorded pid (which is a recyclable walk-up artifact, so a crashed session's pid can read "alive" and show a ghost). A row is shown iff a tool is executing now, OR its transcript was touched within 30 min, OR it is a brand-new session whose transcript file hasn't appeared yet (rescued by an alive pid). Anything quiet longer than the window drops — and reappears the instant it does anything. (The registry **reaper** that deletes rows is separately conservative — pid-identity + a 12h ceiling — since deletion, unlike hiding, isn't free to get wrong.)
