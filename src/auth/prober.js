// auth/prober.js — opt-in background quota probe (DL-3105, upstream #49).
//
// DISABLED BY DEFAULT. When enabled (config.quotaProbeSeconds > 0), periodically
// reads each OAuth account's ZERO-SPEND /api/oauth/usage endpoint so idle accounts'
// utilization/reset stay fresh without waiting to rotate real traffic onto them.
// A sanctioned active-upstream feature (the other is the opt-in keep-warm scheduler,
// auth/warmer.js); the proxy is otherwise passive. Unlike keep-warm, this probe reads
// a zero-spend endpoint and NEVER consumes message quota.
//
// REACTIVE-ONLY BOUNDARY: probe results feed REPORTING only — the manager applies
// them via applyProbeUsage (core/quota.js), which writes utilization/reset for the
// pace line + capacity view but touches NO admission signal and counts NO request.
// Admission still flips only on a real 429. Reads feed capacity truth (refresh-all,
// proactive premium budgets — DL-3017/DL-2841); they never gate admission.
//   • covered by test/quota-probe.test.mjs.
...
export class Prober {
  ...
  /** Change interval at runtime (0 = off). Probes once immediately on off→on. */
  reschedule(intervalMs) {
    ...
    if (intervalMs > 0) {
      ...
      // Immediate probe only on an off→on transition — not on every interval change
      // (mirrors warmer.js; avoids an extra burst when the interval is edited).
  ...
  async probeAccount(account) {
    ...
    try {
      ...
      // Reporting only — applyProbeUsage never touches an admission signal (see
      // core/quota.js) and never counts a request.
      this.am.applyProbeUsage(account.index, usage);
      const finishedAt = Date.now();
      this._recordAccount(account, {
        status: 'ok',
        error: null,
